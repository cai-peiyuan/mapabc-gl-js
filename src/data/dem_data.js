...
// loaded from a image tile, we decode the pixel values using the mapabc terrain-rgb tileset decoding formula, but we store the
...
export default class DEMData {
    ...
    loadFromImage(data: RGBAImage, encoding: "mapabc" | "terrarium") {
        if (data.height !== data.width) throw new RangeError('DEM tiles must be square');
        if (encoding && encoding !== "mapabc" && encoding !== "terrarium") return warnOnce(
            `"${encoding}" is not a valid encoding type. Valid types include "mapabc" and "terrarium".`
        );
        // Build level 0
        const level = this.level = new Level(data.width, data.width / 2);
        const pixels = data.data;

        this._unpackData(level, pixels, encoding || "mapabc");
    ...
    _unpackData(level: Level, pixels: Uint8Array | Uint8ClampedArray, encoding: string) {
        const unpackFunctions = {"mapabc": this._unpackMapbox, "terrarium": this._unpackTerrarium};
