...
test('LogoControl appears in bottom-left by default', (t) => {
    ...
    map.on('load', () => {
        t.equal(map.getContainer().querySelectorAll(
            '.mapabcgl-ctrl-bottom-left .mapabcgl-ctrl-logo'
        ).length, 1);
        t.end();
    });
});
...
test('LogoControl appears in the position specified by the position option', (t) => {
    ...
    map.on('load', () => {
        t.equal(map.getContainer().querySelectorAll(
            '.mapabcgl-ctrl-top-left .mapabcgl-ctrl-logo'
        ).length, 1);
        t.end();
    });
});
...
test('LogoControl is not added more than once', (t)=>{
    ...
    map.on('load', ()=>{
        t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-logo').length, 1, 'first LogoControl');
        map.addSource('source2', source);
        map.on('sourcedata', (e)=>{
            if (e.isSourceLoaded && e.sourceId === 'source2' && e.sourceDataType === 'metadata') {
                t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-logo').length, 1, 'only one LogoControl is added with multiple sources');
                t.end();
            }
        });
    });
...
test('LogoControl appears in compact mode if container is less then 250 pixel wide', (t) => {
    ...
    map.resize();
    t.equal(container.querySelectorAll('.mapabcgl-ctrl-logo:not(.mapabcgl-compact)').length, 1);

    Object.defineProperty(map.getCanvasContainer(), 'offsetWidth', {value: 245, configurable: true});
    map.resize();
    t.equal(container.querySelectorAll('.mapabcgl-ctrl-logo.mapabcgl-compact').length, 1);
