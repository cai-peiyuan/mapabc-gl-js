...
test('GeolocateControl watching map updates recenter on location with dot', (t) => {
    ...
    map.once('moveend', () => {
        ...
        t.false(geolocate._userLocationDotMarker._element.classList.contains('mapabcgl-user-location-dot-stale'), 'userLocation does not have stale class');
        map.once('moveend', () => {
            t.deepEqual(lngLatAsFixed(map.getCenter(), 4), { lat: 40, lng: 50 }, 'map centered on location after 2nd update');
            geolocate.once('error', () => {
                t.ok(geolocate._userLocationDotMarker._map, 'userLocation dot  marker on map');
                t.ok(geolocate._userLocationDotMarker._element.classList.contains('mapabcgl-user-location-dot-stale'), 'userLocation has stale class');
                t.end();
            });
