...
test('FullscreenControl appears when fullscreen is enabled', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-fullscreen').length, 1);
    t.end();
});
...
test('FullscreenControl does not appears when fullscreen is not enabled', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-fullscreen').length, 0);
