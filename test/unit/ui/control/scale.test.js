...
test('ScaleControl appears in bottom-left by default', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-bottom-left .mapabcgl-ctrl-scale').length, 1);
    t.end();
});
...
test('ScaleControl appears in the position specified by the position option', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-ctrl-top-left .mapabcgl-ctrl-scale').length, 1);
    t.end();
});
...
test('ScaleControl should change unit of distance after calling setUnit', (t) => {
    ...
    const selector = '.mapabcgl-ctrl-bottom-left .mapabcgl-ctrl-scale';
