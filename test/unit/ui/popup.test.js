...
test('Popup#addTo adds a .mapabcgl-popup element', (t) => {
    ...
    t.ok(popup.isOpen());
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-popup').length, 1);
    t.end();
});
...
test('Popup closes on close button click events', (t) => {
    ...
    simulateClick(map.getContainer().querySelector('.mapabcgl-popup-close-button'));
...
test('Popup has no close button if closeButton option is false', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-popup-close-button').length, 0);
    t.end();
});
...
test('Popup content can be set via setText', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup').textContent, "Test");
    t.end();
});
...
test('Popup content can be set via setHTML', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup-content').innerHTML, "<span>Test</span>");
    t.end();
});
...
test('Popup content can be set via setDOMContent', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup-content').firstChild, content);
    t.end();
});
...
test('Popup#setText protects against XSS', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup').textContent, "<script>alert('XSS')</script>");
    t.end();
});
...
test('Popup content setters overwrite previous content', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup').textContent, "Test 1");

    popup.setHTML("Test 2");
    t.equal(map.getContainer().querySelector('.mapabcgl-popup').textContent, "Test 2");

    popup.setDOMContent(window.document.createTextNode("Test 3"));
    t.equal(map.getContainer().querySelector('.mapabcgl-popup').textContent, "Test 3");
...
test('Popup anchors as specified by the anchor option', (t) => {
    ...
    t.ok(popup._container.classList.contains('mapabcgl-popup-anchor-top-left'));
    t.end();
});
...
[
    ...
    test(`Popup automatically anchors to ${anchor}`, (t) => {
        ...
        t.ok(popup._container.classList.contains(`mapabcgl-popup-anchor-${anchor}`));
        t.end();
    });
...
test('Popup automatically anchors to top if its bottom offset would push it off-screen', (t) => {
    ...
    t.ok(popup._container.classList.contains('mapabcgl-popup-anchor-top'));
    t.end();
});
...
test('Popup#addTo is idempotent (#1811)', (t) => {
    ...
    t.equal(map.getContainer().querySelector('.mapabcgl-popup-content').textContent, "Test");
    t.end();
});
...
test('Popup#remove is idempotent (#2395)', (t) => {
    ...
    t.equal(map.getContainer().querySelectorAll('.mapabcgl-popup').length, 0);
    t.end();
});
...
test('Popup adds classes from className option', (t) => {
    ...
    const popupContainer = map.getContainer().querySelector('.mapabcgl-popup');
